...
def _compare_stop(cursor, model, stop_id, stop_extid, ext_title, title,
                  shorter, junction, lat, lng):
    try:
        lat = float(lat)
        lng = float(lng)
    except ValueError:
    ...
    existing = model.find_stop(cursor, stop_id)
    if existing[1] != title:
...
def _insert_stop(cursor, model, stop_extid, ext_title, title, shorter,
                 junction, lat, lng):
    try:
        lat = float(lat)
        lng = float(lng)
...
def _compare_route(cursor, model, route_id, route_extid, ext_title, direction):
    existing_route = model.find_route(cursor, route_id, ["line_id"])
    import ipdb; ipdb.set_trace()
    existing_line = model.find_line(cursor, existing_route["line_id"],
                                    ["ext_title"])
...
def _insert_line(cursor, model, ext_title):
    ...
    return model.insert_line(cursor, title, vehicle_type,
                             is_barred=ext_title in barred,
                             ext_title=ext_title)


def _insert_route_stop(cursor, model, route_id, stop_id, stop_index,
                       is_invalid):
    is_enabled = {"true": False, "": True, "false": True}
    if is_invalid not in is_enabled:
        print("Found strage entry in invalid field.")
    return model.insert_route_stop(cursor, route_id, stop_id, stop_index,
                                   is_enabled[is_invalid])


def import_big_csv(data, cursor, model=None):
    if not model:
        import tpt.db
        model = tpt.db
    route_dict = {}
    ...
    for row in csv.reader(data, delimiter=',', quotechar='"'):
        ...
        if stop_extid in stop_dict:
            stop_id = _compare_stop(
                cursor, model, stop_dict[stop_extid], stop_extid,
                row[3].strip(), stop_title, row[5].strip(), row[6].strip(),
                row[7].strip(), row[8].strip())
        else:
            stop_id = _insert_stop(
                cursor, model, stop_extid, row[3].strip(), stop_title,
        ...
        if route_key in route_dict:
            route_id = _compare_route(
                cursor, model, route_dict[route_key], route_extid,
                row[1].strip(), route_dir)
        else:
            ext_linetitle = row[1].strip()
            if ext_linetitle in line_dict:
                line_id = line_dict[ext_linetitle]
            else:
                line_id = _insert_line(cursor, model, ext_linetitle)
                line_dict[ext_linetitle] = line_id
            route_stop_index = 0
            route_id = model.insert_route(cursor, line_id, route_dir,
                                          ext_routeid=route_extid)

            route_dict[route_key] = route_id

        _insert_route_stop(cursor, model, route_id, stop_id, route_stop_index,
                           row[9].strip().lower())
        route_stop_index += 1
