	...
	public int updateStation(IPrefs prefs, int ec) {
		try {
			if(ec<3) {
				...
				//String[] t = RATT.downloadTimes(prefs, extLineId, station.getExtId());
				String[] t = RATT.downloadTimes2(prefs, path.getLineName(), extLineId, station.getName(), extStationId);
